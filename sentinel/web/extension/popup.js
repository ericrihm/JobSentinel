...
function renderResult(data) {
  ...
  if (result.ai_tier_used || result.source) {
    srcEl.textContent = result.ai_tier_used === "client-fallback"
      ? "offline mode"
      : (result.ai_tier_used || result.source || "");
    srcEl.style.display = "inline";
  } else {
    srcEl.style.display = "none";
  }

  // Signals — handle both flat signal arrays and pre-partitioned red_flags/warnings/etc.
  const rawSignals = result.signals ?? result.signal_results ?? [];
  const redFlags  = result.red_flags       ?? rawSignals.filter(s => s.category === "red_flag" && s.triggered !== false);
  const warnings  = result.warnings        ?? rawSignals.filter(s => s.category === "warning"  && s.triggered !== false);
  const positives = result.positive_signals ?? rawSignals.filter(s => s.category === "positive" && s.triggered !== false);
  ...
  // Full report link
  if (data.lastUrl) {
    const link = document.getElementById("link-full-report");
    const encoded = encodeURIComponent(data.lastUrl);
    link.href = `https://jobsentinel.org/analyze.html?url=${encoded}`;
  }

  // Timestamp
  if (data.lastUpdated) {
    const ts = new Date(data.lastUpdated);
    document.getElementById("timestamp").textContent = `Analyzed ${ts.toLocaleTimeString()}`;
  }
...
function renderSignalGroup(sectionId, listId, signals) {
  ...
  signals.forEach((sig) => {
    ...
    li.textContent = sig.detail ?? sig.description ?? sig.name ?? sig.signal_name ?? JSON.stringify(sig);
    list.appendChild(li);
  });
...
// ---------------------------------------------------------------------------
// Trigger analysis on the active tab
// ---------------------------------------------------------------------------

async function triggerAnalysis(tab) {
  showState("loading");
  try {
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        // Reset lastUrl so content.js re-analyzes
        if (typeof lastUrl !== "undefined") {
          // eslint-disable-next-line no-global-assign
          lastUrl = "";
        }
        window.dispatchEvent(new CustomEvent("jobsentinel-reanalyze"));
      },
    });
  } catch (_) {
    // scripting not always available; send a message instead
...
function pollForResult(url, maxAttempts = 20) {
  let attempts = 0;
  const poll = setInterval(async () => {
    ...
    if (fresh.lastResult && fresh.lastUrl === url) {
      ...
      clearInterval(poll);
      document.getElementById("error-msg").textContent = `Analysis error: ${fresh.lastError}`;
      showState("error");
    } else if (attempts >= maxAttempts) {
      clearInterval(poll);
      document.getElementById("error-msg").textContent = "Analysis timed out. Is the page a LinkedIn job posting?";
      showState("error");
    }
  }, 500);
}
...
async function sendReport(verdict) {
  const data = await chrome.storage.local.get(["lastUrl", "lastResult", "apiBase"]);
  if (!data.lastUrl) return;

  const apiBase = data.apiBase || "https://api.jobsentinel.org";
...
// "Analyze Current Page" in no-job state
document.getElementById("btn-analyze").addEventListener("click", async () => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (tab) triggerAnalysis(tab);
});

// "Re-scan" in result state
document.getElementById("btn-analyze-result").addEventListener("click", async () => {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (tab) {
    // Invalidate cache for this URL
    const key = `cache_${tab.url}`;
...
async function init() {
  ...
  const POPUP_TTL_MS = 5 * 60 * 1000; // 5 min — popup shows cached; background has 24h TTL
  const resultFresh =
    data.lastResult &&
    data.lastUrl === url &&
    data.lastUpdated &&
    Date.now() - data.lastUpdated < POPUP_TTL_MS;
  ...
  if (data.lastError && data.lastUrl === url) {
    document.getElementById("error-msg").textContent = `API error: ${data.lastError}`;
    showState("error");
    return;
  }
